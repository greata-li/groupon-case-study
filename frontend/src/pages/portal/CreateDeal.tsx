  ...
  useEffect(() => {
    fetchProfile()
      .then((raw) => {
        ...
        // Check for prefilled deal data from onboarding
        const prefilledJson = sessionStorage.getItem('prefilled_deal');
        if (prefilledJson) {
          try {
            const extracted = JSON.parse(prefilledJson);
            ...
            if (extracted.selected_services && extracted.selected_services.length > 0) {
              ...
              setForm((prev2) => ({ ...prev2, ...updates, ...dealUpdates }));
