...
/** Clean AI-generated text: strip markdown fences, parse JSON arrays into bullet points */
function cleanHighlights(raw: string): string {
  if (!raw) return '';
  ...
  if (cleaned.startsWith('[')) {
    try {
      ...
      if (Array.isArray(arr)) {
        return arr.map((item: string) => typeof item === 'string' ? item.replace(/^["']|["']$/g, '').trim() : String(item)).join('\n');
      }
    } catch {
      // Not valid JSON, continue
    }
  }
  // Strip leading bullet characters and quotes
  return cleaned.split('\n').map((line) => line.replace(/^[\s\-•*"]+/, '').replace(/["',]+$/, '').trim()).filter(Boolean).join('\n');
}
...
export function CreateDeal() {
  ...
  // Check if coming from onboarding with prefilled data, or editing an existing deal
  const searchParams = new URLSearchParams(window.location.search);
  const fromOnboarding = searchParams.get('from') === 'onboarding';
  const editDealId = searchParams.get('edit');
  const [phase, setPhase] = useState<'chat' | 'builder'>(fromOnboarding || editDealId ? 'builder' : 'chat');
  ...
  // Parse prefilled deal once (survives React StrictMode double-effect)
  const prefilledDealRef = useRef<any>(null);
  if (prefilledDealRef.current === null) {
    const raw = sessionStorage.getItem('prefilled_deal');
    if (raw) {
      try {
        prefilledDealRef.current = JSON.parse(raw);
        sessionStorage.removeItem('prefilled_deal');
      } catch {
        prefilledDealRef.current = undefined;
      }
    } else {
      prefilledDealRef.current = undefined;
    }
  ...
  useEffect(() => {
    fetchProfile()
      .then((raw) => {
        ...
        // Load profile photos for reuse in deal
        if (Array.isArray((raw as any).photos) && (raw as any).photos.length > 0) {
          setProfilePhotos((raw as any).photos);
          // Pre-select profile photos for the deal
          updates.uploadedPhotos = (raw as any).photos;
        }
        ...
        // Check for prefilled deal data from onboarding (parsed once into ref)
        const extracted = prefilledDealRef.current;
        if (extracted) {
          try {
            if (extracted.selected_services && extracted.selected_services.length > 0) {
              const dealServices = recalcAllDiscounts(extracted.selected_services.map((s: any) => ({
                included: true,
                name: s.name,
                regularPrice: Number(s.regular_price) || 0,
                grouponPrice: String(s.groupon_price || 0),
                discountPct: 0,
                voucherCap: String(s.voucher_cap || 50),
                description: '',
              })));
              updates.services = dealServices;
              updates.highlights = cleanHighlights(extracted.highlights || '');
              updates.descriptions = Object.fromEntries(
                Object.entries(extracted.descriptions || {}).map(([k, v]) => [parseInt(k), v])
              ) as any;
              updates.expiryDays = extracted.expiry_days || 90;
              updates.restrictionNewCustomers = extracted.restrictions?.includes('New customers only') ?? false;
              updates.restrictionAppointment = extracted.restrictions?.includes('Appointment required') ?? true;
  ...
  useEffect(() => {
    ...
    fetchDeal(editDealId)
      .then((published) => {
        const d = published.deal;
        if (!d) return;
  ...
  function recalcAllDiscounts(services: ServiceEntry[]): ServiceEntry[] {
    return services.map((svc) => {
      const reg = Number(svc.regularPrice) || 0;
      const grp = Number(svc.grouponPrice) || 0;
  ...
  function handleDealExtracted(extracted: ExtractedDeal) {
    if (extracted.selected_services && extracted.selected_services.length > 0) {
      setForm((prev) => ({
        ...prev,
  ...
  const handleInspireMe = useCallback(
    async (field: 'highlights' | 'description', serviceIndex?: number) => {
      ...
      try {
        ...
        // Collect existing descriptions so LLM can avoid repetition
        const existingDescriptions = field === 'description'
          ? Object.entries(form.descriptions)
              .filter(([k, v]) => v && parseInt(k) !== serviceIndex)
              .map(([, v]) => v)
          : [];
        ...
        const result = await enhanceText(currentText, field, {
          ...
          ...(existingDescriptions.length > 0 ? { existing_descriptions: existingDescriptions } : {}),
        });

        const enhanced = result?.enhanced_text ?? currentText;

        if (field === 'highlights') {
          updateForm({ highlights: cleanHighlights(enhanced) });
        ...
        {dealAssistUsed && (
          <div className="flex items-center gap-2 rounded-lg bg-groupon-green-light/30 border border-groupon-green/20 px-4 py-2.5">
            ...
            <span className="text-sm text-groupon-green font-medium">
              AI pre-filled your deal from your description. Review and adjust below.
            </span>
          </div>
        )}
    ...
    return (
      <div className="space-y-6">
        ...
        {form.photoOption === 'upload' && (
          <div>
            {/* Show existing photos (from profile or previous uploads) */}
            {form.uploadedPhotos.length > 0 && (
              <div className="mb-4">
                <p className="text-xs font-medium text-gray-500 mb-2">
                  {profilePhotos.length > 0 ? 'Photos from your profile' : 'Uploaded photos'}
                </p>
            ...
            <div
              className="rounded-xl border-2 border-dashed border-gray-300 bg-gray-50/50 p-8 text-center cursor-pointer hover:border-groupon-green/40 hover:bg-groupon-green/[0.02] transition-all"
              onClick={() => {
                ...
                inp.onchange = async (e) => {
                  ...
                  for (const file of Array.from(files)) {
                    ...
                    if (res.ok) {
                      ...
                      updateForm({ uploadedPhotos: [...form.uploadedPhotos, url] });
                    }
                  }
                };
                inp.click();
            ...
            >
              ...
              <p className="text-sm font-medium text-gray-700">
                {form.uploadedPhotos.length > 0 ? 'Add more photos' : 'Drag and drop photos here'}
              </p>
              <p className="text-xs text-gray-400 mt-1">PNG, JPG up to 10MB each</p>
              <Button variant="outline" className="mt-3 rounded-lg text-xs" onClick={(e) => e.stopPropagation()}>
                Choose Files
              </Button>
            </div>
          </div>
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="mt-3 flex gap-2">
            <FeatureTooltip id="inspire-me" message="Let AI write or improve your copy. Click to generate highlights, descriptions, and more." position="bottom">
              <Button
                variant="outline"
                size="sm"
  ...
  // Show conversational chat phase first
  if (phase === 'chat') {
    return (
      <DealChat
    ...
    <div className="animate-fade-in-up p-4 sm:p-6">
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1 min-w-0 max-w-[700px]">
          {/* Success feedback */}
          {saveSuccess && (
            <div className="mb-4 flex items-center gap-2 rounded-xl bg-groupon-green-light border border-groupon-green/20 px-4 py-3 animate-fade-in-up">
              <CheckCircle2 className="h-5 w-5 text-groupon-green" />
              <span className="text-sm font-medium text-groupon-green">
                {editDealId ? 'Deal updated successfully!' : 'Deal published successfully!'} Redirecting...
              </span>
            </div>
          )}
